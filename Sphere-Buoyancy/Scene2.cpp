...
Scene2::Scene2() : sphereDynamic(nullptr), sphereStatic(nullptr), meshPtr(nullptr), shaderPtr(nullptr), texturePtr(nullptr)
{
...
bool Scene2::OnCreate() {
	...
	sphereDynamic->setPos(Vec3(4.0f, 4.0f, 0.0f));
	sphereDynamic->setRadius(1.12f);
	sphereDynamic->setVel(Vec3(0.0f, -4.0f, 0.0f));

	//Static Sphere Values
	sphereStatic->setPos(Vec3(3.0f, 0.0f, 0.0f));
	sphereStatic->setVel(Vec3(0.0f, 0.0f, 0.0f)); // Zero Velocity
...
void Scene2::Update(const float deltaTime) {
	physics->SphereStaticSphereCollisionResponse(*sphereDynamic, *sphereStatic);
